...
if(isset($_POST['message']) && $_POST['message'] != '') {
	$sql = "INSERT INTO ".$_GET['chname']." (chat_id, user_id, user_name, message, post_time) VALUES (" . 
...
if(isset($_POST['action']) && $_POST['action'] == 'reset') {
	$sql = "DELETE FROM ".$_GET['chname']." WHERE chat_id = " . db_input($_GET['chat']);
	db_query($sql);
}
...
if(!isset($_GET['chat'])) {
	...
	$sql = "SELECT message_id, user_name, message, date_format(post_time, '%h:%i') as post_time" . 
		" FROM ".$_GET['chname']." WHERE chat_id = " . db_input($_GET['chat']) . " AND message_id > " . $last;
